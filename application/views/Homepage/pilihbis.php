		...
		<div class="container">
			<div class="row">
				<table id="tabel_bus" class="table table-bordered table-striped">
					...
					<?php foreach ($bis as $key) {?>
					<tr>
						<td><?php echo $key->nama_bus?></td>
						<td><?php echo $key->tipe_bus?></td>
						<td><?php echo $key->jam_berangkat?></td>
						<td><?php echo $key->harga_tiket?></td>
						<td><?php echo $key->stok?></td>
						<input type="hidden" name="kode" value="<?php echo $key->kode_tiket?>">
						<td>
							<a href="<?php echo base_url('Welcome/data_penumpang')?>" class="btn btn-success"> Beli </a>
						</td>
					</tr>
					<?php } ?>
				</table>
